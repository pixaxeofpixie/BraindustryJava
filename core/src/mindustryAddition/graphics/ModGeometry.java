...
public class ModGeometry {
    ...
    public static Seq<Vec2> getRectPoints(float x, float y, float width, float height, float rot){
        float vw = width / 2f;
        float vh = height / 2f;
...
//        if (cords.size>5)cords.remove(cords.size-1);
    ...
    public static Seq<Vec2> getRectPoints(Rect rect, float rot){
        return getRectPoints(rect.x,rect.y,rect.width,rect.height,rot);
    }
}
